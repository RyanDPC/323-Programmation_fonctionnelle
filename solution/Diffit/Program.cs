...
var comparison = linesA
    .Select((line, i) => new LinesComparison
    {
        Number = i,
        ContentA = line,
        ContentB = linesB[i]
...
var diffLines = comparison.Where(line => line.ContentA != line.ContentB);

// TODO: 08 Afficher le nombre de lignes identiques et différentes entre les 2 fichiers
Console.WriteLine($"Nombre de lignes différentes : {diffLines.Count()}");
Console.WriteLine($"Nombre de lignes identiques : {comparison.Length - diffLines.Count()}");
...
Func<LinesComparison, int> countVariations = line => line.ContentA.Zip(line.ContentB, (a,b) => a==b?0:1).Sum() + line.LengthVariation;
